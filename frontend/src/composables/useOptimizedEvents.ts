...
export function useOptimizedEvents() {
  ...
  const addOptimizedEventListener = <K extends keyof WindowEventMap | string>(
    ...
    if (debounceMs) {
      optimizedListener = debounce(listener as EventListener, debounceMs) as EventListenerOrEventListenerObject;
    } else if (throttleMs) {
      optimizedListener = throttle(listener as EventListener, throttleMs) as EventListenerOrEventListenerObject;
    }
  ...
  const addVisibilityChangeListener = (
    ...
    return addOptimizedEventListener(document, 'visibilitychange' as any, listener as EventListener, options);
  };
