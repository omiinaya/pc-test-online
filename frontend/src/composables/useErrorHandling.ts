...
export function useErrorHandling(componentName: string = 'Component'): UseErrorHandlingReturn {
  ...
  const errorMessages: ErrorMessages = {
    ...
    DEVICE_BUSY: (deviceType: string): string => `${deviceType} is already in use by another application.`,
    DEVICE_TIMEOUT: (deviceType: string): string =>
    ...
    BROWSER_NOT_SUPPORTED: (feature: string): string => `${feature} is not supported in this browser.`,
    HTTPS_REQUIRED: (): string =>
