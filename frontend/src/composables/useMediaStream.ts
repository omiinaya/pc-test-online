...
export function useMediaStream(): UseMediaStreamReturn {
    ...
    const createStream = async (constraints: DeviceConstraints): Promise<MediaStream> => {
        ...
        try {
            ...
            console.log(
                '[useMediaStream] Stream video tracks:',
                stream.value.getVideoTracks().length
            );
            console.log(
                '[useMediaStream] Stream audio tracks:',
                stream.value.getAudioTracks().length
            );
    ...
    const stopStream = (): void => {
        ...
        if (stream.value) {
            ...
            stream.value.getTracks().forEach(track => {
                console.log(
                    '[useMediaStream] Disabling track:',
                    track.label,
                    'enabled:',
                    track.enabled
                );
    ...
    const switchDevice = async (
        ...
        try {
            ...
            const baseConstraint = baseConstraints[constraintKey] as
                | MediaTrackConstraints
                | boolean
                | undefined;
            const newConstraint: MediaTrackConstraints =
                typeof baseConstraint === 'object' && baseConstraint !== null
