...
export interface StatePanelConfigs {
  [key: string]: StatePanelConfig;
}

export interface UseStatePanelConfigsReturn {
  // All configurations
  configs: StatePanelConfigs;

  // Methods
  getConfig: (stateName: string, customProps?: Partial<StatePanelConfig>) => StatePanelConfig;
  getConfigForDevice: (
    stateName: string,
    customDeviceType: string,
    customProps?: Partial<StatePanelConfig>
  ) => StatePanelConfig;
  getErrorConfig: (error: unknown, customProps?: Partial<StatePanelConfig>) => StatePanelConfig | null;
}

/**
 * Composable for standardized StatePanel configurations
 */
export function useStatePanelConfigs(deviceType: string = 'device'): UseStatePanelConfigsReturn {
   ...
   */
  const getConfig = (stateName: string, customProps: Partial<StatePanelConfig> = {}): StatePanelConfig => {
   ...
   */
  const getErrorConfig = (error: unknown, customProps: Partial<StatePanelConfig> = {}): StatePanelConfig | null => {
