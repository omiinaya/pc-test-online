...
export function useMediaPermissions(
  ...
  const initializePermissions = async (): Promise<void> => {
    ...
    console.log(`[useMediaPermissions:${deviceType}] Background permission check result:`, permissionState);

    // If we got a definitive answer, use it
    if (permissionState) {
      console.log(`[useMediaPermissions:${deviceType}] Handling permission state:`, permissionState);
      handlePermissionState(permissionState);
    } else {
      // Fallback: assume we need to request permission
      console.log(`[useMediaPermissions:${deviceType}] No definitive permission state, showing permission prompt`);
  ...
  const requestPermission = async (constraints: DeviceConstraints): Promise<MediaStream> => {
    console.log(`[useMediaPermissions:${deviceType}] requestPermission() called with constraints:`, constraints);
    try {
      // Standard browser flow
      console.log(`[useMediaPermissions:${deviceType}] Calling navigator.mediaDevices.getUserMedia()`);
      const stream = await navigator.mediaDevices.getUserMedia(constraints);
      console.log(`[useMediaPermissions:${deviceType}] Permission granted, stream created:`, stream);
      permissionGranted.value = true;
      permissionDenied.value = false;
      console.log(`[useMediaPermissions:${deviceType}] permissionGranted:`, permissionGranted.value);
      return stream;
    } catch (err: unknown) {
